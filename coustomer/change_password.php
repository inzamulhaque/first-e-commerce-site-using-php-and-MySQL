...
<?php
    ...
    if (isset($_POST['update'])) {
        ...
        if ($check_old_pass==0) {
            echo "<script>alert('Your current password is not valid... try agein')</script>";
            exit();
        }
        if ($new_password!=$c_n_password) {
            echo "<script>alert('Your new password does not match with your confirm password')</script>";
            exit();
        }
